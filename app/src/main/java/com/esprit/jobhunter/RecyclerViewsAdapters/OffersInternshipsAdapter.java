...
public class OffersInternshipsAdapter extends RecyclerView.Adapter<OffersInternshipsAdapter.ViewHolder> {

    private Context context;
    private List<Internship> list ;

    public OffersInternshipsAdapter(Context context, ArrayList<Internship> list) {
        this.context = context;
        this.list = list;
    }

    @Override
    public OffersInternshipsAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.offer_job_list_item, parent, false);
        return new OffersInternshipsAdapter.ViewHolder(v);
    }

    @Override
    public void onBindViewHolder(OffersInternshipsAdapter.ViewHolder holder, int position) {
        Internship job = list.get(position);
        final int x = position;

        holder.textLabel.setText(job.getLabel());
        holder.type.setText(job.getDuration());
